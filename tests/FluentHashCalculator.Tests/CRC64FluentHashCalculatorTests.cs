...
namespace FluentHashCalculator.Tests
{
    public class CRC64FluentHashCalculatorTests
    {
        ...
        [Fact]
        public (ulong, ulong) UsingAllPropertiesInCalculatorWhenComputeThenReturnCRC64()
        {
            var calculator = new AbstractHashCalculatorBuilder<EntityWithAllSupportedTypes>.CRC64();
            calculator.Using(e => e.BoolProperty)
        ...
        [Fact]
        public (ulong, ulong, ulong) UsingComplexPropertyWithStringInCalculatorWhenComputeThenReturnEntityIdWithSameEncodingToParentCalculatorCRC64()
        {
            // Tests without context inheritance
            var calculator = new AbstractHashCalculatorBuilder<EntityWithAllSupportedTypes>.CRC64();
            calculator.WithEncoding(Encoding.Unicode).Using(e => e.Child).WithCRC64(calc => calc.WithEncoding(Encoding.UTF8).Using(e => e.Name));
            var utf8Hash = calculator.Compute(new EntityWithAllSupportedTypes());
           Assert.Equal(Consts.CHILD_ENTITY_STRING_UTF8_CRC64, utf8Hash);
            calculator.Context.Encoding = Encoding.Unicode;
            var unicodeHash = calculator.Compute(new EntityWithAllSupportedTypes());
           Assert.Equal(Consts.CHILD_ENTITY_STRING_UTF8_CRC64, unicodeHash);
            calculator.Context.Encoding = Encoding.UTF32;
            var utf32Hash = calculator.Compute(new EntityWithAllSupportedTypes());
           Assert.Equal(Consts.CHILD_ENTITY_STRING_UTF8_CRC64, utf32Hash);

            // Tests with context inheritance
            calculator = new AbstractHashCalculatorBuilder<EntityWithAllSupportedTypes>.CRC64();
            calculator.Using(e => e.Child, inheritContext: true).WithCRC64(calc => calc.Using(e => e.Name));
           ...
           Assert.Equal(Consts.CHILD_ENTITY_STRING_UNICODE_CRC64, unicodeHash);
            calculator.Context.Encoding = Encoding.UTF32;
            utf32Hash = calculator.Compute(new EntityWithAllSupportedTypes());
           Assert.Equal(Consts.CHILD_ENTITY_STRING_UTF32_CRC64, utf32Hash);

            return (utf8Hash, unicodeHash, utf32Hash);
        }

        [Fact]
        public (ulong, ulong, ulong) UsingComplexPropertyListWithStringInCalculatorWhenComputeThenReturnEntityIdWithSameEncodingToParentCalculatorCRC64()
        {
            // Tests without context inheritance
            var calculator = new AbstractHashCalculatorBuilder<EntityWithAllSupportedTypes>.CRC64();
            calculator
                .WithEncoding(Encoding.Unicode)
                .WithErrorHandling(ErrorHandling.Ignore)
                .UsingEach(e => e.ChildList).WithCRC64(calc => calc.WithEncoding(Encoding.UTF8).Using(e => e.Name));
            var utf8Hash = calculator.Compute(new EntityWithAllSupportedTypes());
           Assert.Equal(Consts.CHILDLIST_ENTITY_STRING_UTF8_CRC64, utf8Hash);
            calculator.Context.Encoding = Encoding.Unicode;
            var unicodeHash = calculator.Compute(new EntityWithAllSupportedTypes());
           Assert.Equal(Consts.CHILDLIST_ENTITY_STRING_UTF8_CRC64, unicodeHash);
            calculator.Context.Encoding = Encoding.UTF32;
            var utf32Hash = calculator.Compute(new EntityWithAllSupportedTypes());
           Assert.Equal(Consts.CHILDLIST_ENTITY_STRING_UTF8_CRC64, utf32Hash);

            // Tests with context inheritance
            calculator = new AbstractHashCalculatorBuilder<EntityWithAllSupportedTypes>.CRC64();
            calculator.UsingEach(e => e.ChildList, inheritContext: true).WithCRC64(calc => calc.Using(e => e.Name));
            utf8Hash = calculator.Compute(new EntityWithAllSupportedTypes());
           Assert.Equal(Consts.CHILDLIST_ENTITY_STRING_UTF8_CRC64, utf8Hash);
            calculator.Context.Encoding = Encoding.Unicode;
            unicodeHash = calculator.Compute(new EntityWithAllSupportedTypes());
           Assert.Equal(Consts.CHILDLIST_ENTITY_STRING_UNICODE_CRC64, unicodeHash);
            calculator.Context.Encoding = Encoding.UTF32;
            utf32Hash = calculator.Compute(new EntityWithAllSupportedTypes());
           Assert.Equal(Consts.CHILDLIST_ENTITY_STRING_UTF32_CRC64, utf32Hash);
